// <auto-generated />
using System;
using LeadEngine.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace LeadEngine.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(LeadEngineDbContext))]
    [Migration("20260907030435_AddCreativeAssetSoftDelete")]
    partial class AddCreativeAssetSoftDelete
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.7")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("LeadEngine.Domain.Entities.Campanha", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<bool>("Ativo")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("BeneficiosJson")
                        .HasColumnType("json");

                    b.Property<string>("CampaignConfigJson")
                        .HasColumnType("json");

                    b.Property<string>("Cidade")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataDespublicacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataGeracao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataPublicacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("DescricoesAnunciosJson")
                        .HasColumnType("json");

                    b.Property<long?>("DuracaoGeracaoMs")
                        .HasColumnType("bigint");

                    b.Property<string>("ErroGeracao")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("varchar(2)");

                    b.Property<string>("MensagemWhatsApp")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("ModeloIa")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Objetivo")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("Operadora")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<decimal>("OrcamentoDiario")
                        .HasPrecision(10, 2)
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("PalavrasChaveJson")
                        .HasColumnType("json");

                    b.Property<string>("PalavrasChaveNegativasJson")
                        .HasColumnType("json");

                    b.Property<string>("PerguntasFrequentesJson")
                        .HasColumnType("json");

                    b.Property<string>("ProviderIa")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<bool>("Publicada")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Regiao")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<Guid?>("SegmentId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<string>("SubtituloLandingPage")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("TextoBotao")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<int>("TipoPublico")
                        .HasColumnType("int");

                    b.Property<string>("TituloLandingPage")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("TitulosAnunciosJson")
                        .HasColumnType("json");

                    b.Property<string>("UrlPublica")
                        .HasMaxLength(250)
                        .HasColumnType("varchar(250)");

                    b.HasKey("Id");

                    b.HasIndex("DataCriacao");

                    b.HasIndex("SegmentId");

                    b.HasIndex("Slug")
                        .IsUnique();

                    b.HasIndex("Status");

                    b.HasIndex("Publicada", "Ativo");

                    b.ToTable("Campanhas", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CampanhaRevisao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("ConteudoAnterior")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("ConteudoNovo")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<DateTime>("DataAlteracao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("InstrucaoAdicional")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("ModeloIa")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<int>("Origem")
                        .HasColumnType("int");

                    b.Property<string>("ProviderIa")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<int?>("Secao")
                        .HasColumnType("int");

                    b.Property<string>("TipoAlteracao")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.HasKey("Id");

                    b.HasIndex("CampanhaId");

                    b.HasIndex("DataAlteracao");

                    b.ToTable("CampanhasRevisoes", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.ConfiguracaoSistema", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<bool>("Ativo")
                        .HasColumnType("tinyint(1)");

                    b.Property<int>("Categoria")
                        .HasColumnType("int");

                    b.Property<string>("Chave")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Descricao")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<bool>("Sensivel")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Valor")
                        .HasMaxLength(2000)
                        .HasColumnType("varchar(2000)");

                    b.Property<string>("ValorProtegido")
                        .HasMaxLength(4000)
                        .HasColumnType("varchar(4000)");

                    b.HasKey("Id");

                    b.HasIndex("Categoria");

                    b.HasIndex("Chave")
                        .IsUnique();

                    b.ToTable("ConfiguracoesSistema", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.ConfiguracaoSistemaHistorico", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<int>("Categoria")
                        .HasColumnType("int");

                    b.Property<string>("Chave")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<Guid>("ConfiguracaoSistemaId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("DataAlteracao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("OrigemAlteracao")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<bool>("Sensivel")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("ValorAnterior")
                        .HasMaxLength(2000)
                        .HasColumnType("varchar(2000)");

                    b.Property<string>("ValorNovo")
                        .HasMaxLength(2000)
                        .HasColumnType("varchar(2000)");

                    b.HasKey("Id");

                    b.HasIndex("Categoria");

                    b.HasIndex("Chave");

                    b.HasIndex("ConfiguracaoSistemaId");

                    b.HasIndex("DataAlteracao");

                    b.ToTable("ConfiguracoesSistemaHistorico", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeAsset", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("CampaignId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<long>("FileSize")
                        .HasColumnType("bigint");

                    b.Property<int>("Height")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("IsSelected")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("MimeType")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("StoragePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int>("Width")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CampaignId");

                    b.HasIndex("CreatedAt");

                    b.HasIndex("IsDeleted");

                    b.HasIndex("CampaignId", "IsSelected");

                    b.ToTable("CreativeAssets", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeAssetAnalysis", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<int>("BrandFitScore")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("CreativeAssetId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Model")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("PlacementRecommendationsJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("Provider")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<string>("RawResponseJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("RisksJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("SuggestedCta")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<string>("SuggestedDescription")
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("SuggestedHeadline")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("SuggestedPrimaryText")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("Summary")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("varchar(2000)");

                    b.Property<int>("TextDensityScore")
                        .HasColumnType("int");

                    b.Property<int>("VisualQualityScore")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CreatedAt");

                    b.HasIndex("CreativeAssetId");

                    b.ToTable("CreativeAssetAnalyses", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeQualityOverride", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("CampaignId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid?>("CreativeAssetAnalysisId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("CreativeAssetId")
                        .HasColumnType("char(36)");

                    b.Property<int?>("RankingScore")
                        .HasColumnType("int");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<bool>("SemanticMismatch")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("User")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.HasKey("Id");

                    b.HasIndex("CampaignId");

                    b.HasIndex("CreatedAt");

                    b.HasIndex("CreativeAssetAnalysisId");

                    b.HasIndex("CreativeAssetId");

                    b.ToTable("CreativeQualityOverrides", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsAnaliseIa", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<bool>("Aplicada")
                        .HasColumnType("tinyint(1)");

                    b.Property<decimal?>("CustoEstimado")
                        .HasPrecision(12, 6)
                        .HasColumnType("decimal(12,6)");

                    b.Property<DateTime?>("DataAplicacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<long>("DuracaoMs")
                        .HasColumnType("bigint");

                    b.Property<Guid>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Modelo")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<DateOnly>("PeriodoFinal")
                        .HasColumnType("date");

                    b.Property<DateOnly>("PeriodoInicial")
                        .HasColumnType("date");

                    b.Property<string>("Provider")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<string>("ResultadoJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("Resumo")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("varchar(2000)");

                    b.Property<int?>("TokensEntrada")
                        .HasColumnType("int");

                    b.Property<int?>("TokensSaida")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DataCriacao");

                    b.HasIndex("GoogleAdsPublicacaoId");

                    b.ToTable("GoogleAdsAnalisesIa", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsConta", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("AccessTokenExpiraEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("AccessTokenProtegido")
                        .HasMaxLength(4000)
                        .HasColumnType("varchar(4000)");

                    b.Property<bool>("Ativa")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("CustomerId")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataConexao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Email")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<bool>("Padrao")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("RefreshTokenProtegido")
                        .HasMaxLength(4000)
                        .HasColumnType("varchar(4000)");

                    b.HasKey("Id");

                    b.HasIndex("Ativa");

                    b.HasIndex("CustomerId")
                        .IsUnique();

                    b.HasIndex("Padrao");

                    b.ToTable("GoogleAdsContas", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsMetricaDiaria", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("CampaignExternalId")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("CampaignResourceName")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<long>("Cliques")
                        .HasColumnType("bigint");

                    b.Property<decimal>("Conversoes")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("CpcMedio")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)");

                    b.Property<long>("CpcMedioMicros")
                        .HasColumnType("bigint");

                    b.Property<decimal>("Ctr")
                        .HasPrecision(12, 6)
                        .HasColumnType("decimal(12,6)");

                    b.Property<decimal>("Custo")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)");

                    b.Property<long>("CustoMicros")
                        .HasColumnType("bigint");

                    b.Property<DateOnly>("Data")
                        .HasColumnType("date");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataSincronizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("GoogleAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<long>("Impressoes")
                        .HasColumnType("bigint");

                    b.Property<decimal?>("ParcelaImpressoesPesquisa")
                        .HasPrecision(12, 6)
                        .HasColumnType("decimal(12,6)");

                    b.Property<decimal>("TaxaConversao")
                        .HasPrecision(12, 6)
                        .HasColumnType("decimal(12,6)");

                    b.Property<decimal?>("TaxaTopoAbsoluto")
                        .HasPrecision(12, 6)
                        .HasColumnType("decimal(12,6)");

                    b.Property<decimal?>("TaxaTopoPagina")
                        .HasPrecision(12, 6)
                        .HasColumnType("decimal(12,6)");

                    b.Property<decimal>("ValorConversoes")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)");

                    b.HasKey("Id");

                    b.HasIndex("Data");

                    b.HasIndex("GoogleAdsContaId");

                    b.HasIndex("GoogleAdsPublicacaoId", "CampaignExternalId", "Data")
                        .IsUnique();

                    b.ToTable("GoogleAdsMetricasDiarias", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsOAuthState", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataUtilizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("ExpiraEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("StateHash")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("varchar(128)");

                    b.Property<bool>("Utilizado")
                        .HasColumnType("tinyint(1)");

                    b.HasKey("Id");

                    b.HasIndex("ExpiraEm");

                    b.HasIndex("StateHash")
                        .IsUnique();

                    b.HasIndex("Utilizado");

                    b.ToTable("GoogleAdsOAuthStates", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsOperacaoPublicacao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("CodigoErro")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<DateTime?>("DataConclusao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("EntidadeOrigem")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<Guid>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<int>("Indice")
                        .HasColumnType("int");

                    b.Property<string>("MensagemControlada")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("ResourceNameDefinitivo")
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("ResourceNameTemporario")
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("varchar(60)");

                    b.Property<string>("TipoOperacao")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.HasKey("Id");

                    b.HasIndex("GoogleAdsPublicacaoId");

                    b.HasIndex("GoogleAdsPublicacaoId", "Indice");

                    b.ToTable("GoogleAdsOperacoesPublicacao", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPlanoPublicacao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AvisosValidacaoJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<Guid>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("CodigoMoeda")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("varchar(3)");

                    b.Property<string>("ConteudoHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("varchar(64)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataValidacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ErrosValidacaoJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<Guid>("GoogleAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Idioma")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)");

                    b.Property<string>("NomeCampanha")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Objetivo")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<decimal>("OrcamentoDiario")
                        .HasPrecision(10, 2)
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("Pais")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)");

                    b.Property<string>("PayloadPreviewJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<string>("TipoRede")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("UrlFinal")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int>("Versao")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CampanhaId")
                        .IsUnique();

                    b.HasIndex("GoogleAdsContaId");

                    b.HasIndex("Status");

                    b.ToTable("GoogleAdsPlanosPublicacao", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPublicacao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("ConfirmationExpiresAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ConfirmationTokenHash")
                        .HasMaxLength(128)
                        .HasColumnType("varchar(128)");

                    b.Property<string>("CustomerId")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataConclusao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataInicioPublicacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataPreparacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataValidacaoRemota")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ErroCodigo")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("ErroMensagemControlada")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("ErrosJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("GeoTargetResourceName")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<Guid>("GoogleAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("GoogleAdsPlanoPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<string>("IdempotencyKey")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<bool>("IsTestAccount")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("LanguageResourceName")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("PreviewHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("varchar(64)");

                    b.Property<int>("PreviewVersao")
                        .HasColumnType("int");

                    b.Property<string>("RecursosJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("RequestIdPublicacao")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("RequestIdValidacao")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("Tentativas")
                        .HasColumnType("int");

                    b.Property<bool>("Teste")
                        .HasColumnType("tinyint(1)");

                    b.HasKey("Id");

                    b.HasIndex("CampanhaId");

                    b.HasIndex("GoogleAdsContaId");

                    b.HasIndex("Status");

                    b.HasIndex("GoogleAdsPlanoPublicacaoId", "PreviewVersao", "PreviewHash")
                        .IsUnique();

                    b.ToTable("GoogleAdsPublicacoes", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPublicacaoHistorico", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("Data")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<string>("MensagemControlada")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("MetadadosJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.Property<string>("Operacao")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("RequestId")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<int?>("StatusAnterior")
                        .HasColumnType("int");

                    b.Property<int>("StatusNovo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("Data");

                    b.HasIndex("GoogleAdsPublicacaoId");

                    b.ToTable("GoogleAdsPublicacaoHistoricos", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsRecursoPublicado", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ExternalId")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<Guid>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Nome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("ResourceName")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<string>("TipoRecurso")
                        .IsRequired()
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.HasKey("Id");

                    b.HasIndex("GoogleAdsPublicacaoId");

                    b.HasIndex("ResourceName");

                    b.ToTable("GoogleAdsRecursosPublicados", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsSincronizacao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("DataConclusao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataInicio")
                        .HasColumnType("datetime(6)");

                    b.Property<long>("DuracaoMs")
                        .HasColumnType("bigint");

                    b.Property<string>("ErroCodigo")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("ErroMensagemControlada")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<Guid>("GoogleAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<DateOnly?>("PeriodoFinal")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("PeriodoInicial")
                        .HasColumnType("date");

                    b.Property<int>("RegistrosAtualizados")
                        .HasColumnType("int");

                    b.Property<int>("RegistrosConsultados")
                        .HasColumnType("int");

                    b.Property<int>("RegistrosCriados")
                        .HasColumnType("int");

                    b.Property<string>("RequestId")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DataInicio");

                    b.HasIndex("GoogleAdsContaId");

                    b.HasIndex("GoogleAdsPublicacaoId");

                    b.ToTable("GoogleAdsSincronizacoes", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Lead", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Cep")
                        .HasMaxLength(8)
                        .HasColumnType("varchar(8)");

                    b.Property<string>("Cidade")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("Cnpj")
                        .HasMaxLength(14)
                        .HasColumnType("varchar(14)");

                    b.Property<string>("CnpjNormalizado")
                        .HasMaxLength(14)
                        .HasColumnType("varchar(14)");

                    b.Property<bool>("ConsentimentoContato")
                        .HasColumnType("tinyint(1)");

                    b.Property<DateTime>("ConsentimentoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataAtribuicao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Email")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("EmailNormalizado")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<DateTime?>("EnviadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ErroEnvio")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("Fbclid")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Gclid")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<Guid?>("GoogleAdsPublicacaoId")
                        .HasColumnType("char(36)");

                    b.Property<string>("HospitalDesejado")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("IdadesJson")
                        .HasColumnType("json");

                    b.Property<string>("IpHash")
                        .HasMaxLength(128)
                        .HasColumnType("varchar(128)");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)");

                    b.Property<string>("NomeEmpresa")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Observacao")
                        .HasMaxLength(1000)
                        .HasColumnType("varchar(1000)");

                    b.Property<string>("OperadoraDesejada")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("OrigemCaptura")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<string>("PlanoAtual")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<bool?>("PossuiPlanoAtual")
                        .HasColumnType("tinyint(1)");

                    b.Property<int?>("QuantidadeFuncionarios")
                        .HasColumnType("int");

                    b.Property<int?>("QuantidadeVidas")
                        .HasColumnType("int");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<string>("StatusEnvioExterno")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<int>("TentativasEnvioExterno")
                        .HasColumnType("int");

                    b.Property<string>("TextoConsentimentoVersao")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.Property<int>("TipoAtribuicao")
                        .HasColumnType("int");

                    b.Property<int?>("TipoContratacao")
                        .HasColumnType("int");

                    b.Property<string>("Uf")
                        .HasMaxLength(2)
                        .HasColumnType("varchar(2)");

                    b.Property<string>("UltimoErroEnvioExterno")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int?>("UltimoStatusHttpIntegracao")
                        .HasColumnType("int");

                    b.Property<string>("UserAgentResumo")
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("UtmCampaign")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("UtmContent")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("UtmMedium")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("UtmSource")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("UtmTerm")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("WhatsApp")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("WhatsAppNormalizado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id");

                    b.HasIndex("CampanhaId");

                    b.HasIndex("CriadoEm");

                    b.HasIndex("GoogleAdsPublicacaoId");

                    b.HasIndex("Status");

                    b.HasIndex("Tipo");

                    b.HasIndex("TipoAtribuicao");

                    b.HasIndex("WhatsAppNormalizado");

                    b.HasIndex("CampanhaId", "WhatsAppNormalizado", "CriadoEm");

                    b.ToTable("Leads", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadAnswer", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("FieldKey")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("LabelSnapshot")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<Guid>("LeadId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("ValueJson")
                        .IsRequired()
                        .HasColumnType("json");

                    b.HasKey("Id");

                    b.HasIndex("LeadId");

                    b.HasIndex("LeadId", "FieldKey");

                    b.ToTable("LeadAnswers", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadForm", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("CampaignId")
                        .HasColumnType("char(36)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("SubmitButtonText")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<int>("Version")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CampaignId");

                    b.HasIndex("CampaignId", "IsActive");

                    b.HasIndex("CampaignId", "Version")
                        .IsUnique();

                    b.ToTable("LeadForms", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadFormField", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("DefaultValue")
                        .HasMaxLength(300)
                        .HasColumnType("varchar(300)");

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Label")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<Guid>("LeadFormId")
                        .HasColumnType("char(36)");

                    b.Property<string>("OptionsJson")
                        .HasColumnType("json");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Placeholder")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<bool>("Required")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("ValidationJson")
                        .HasColumnType("json");

                    b.HasKey("Id");

                    b.HasIndex("LeadFormId");

                    b.HasIndex("LeadFormId", "Key")
                        .IsUnique();

                    b.HasIndex("LeadFormId", "Order");

                    b.ToTable("LeadFormFields", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LogIntegracaoLead", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Endpoint")
                        .HasMaxLength(250)
                        .HasColumnType("varchar(250)");

                    b.Property<Guid>("LeadId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Mensagem")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int?>("StatusHttp")
                        .HasColumnType("int");

                    b.Property<bool>("Sucesso")
                        .HasColumnType("tinyint(1)");

                    b.HasKey("Id");

                    b.HasIndex("CriadoEm");

                    b.HasIndex("LeadId");

                    b.ToTable("LogsIntegracaoLead", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsAtivoSelecionado", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AdAccountId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("AdAccountNome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("BusinessId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("BusinessNome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("InstagramAccountId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("InstagramNome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<Guid>("MetaAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("PageId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("PageNome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("PixelId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("PixelNome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.HasKey("Id");

                    b.HasIndex("AdAccountId");

                    b.HasIndex("BusinessId");

                    b.HasIndex("MetaAdsContaId")
                        .IsUnique();

                    b.HasIndex("PageId");

                    b.ToTable("MetaAdsAtivosSelecionados", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsConta", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("AccessTokenExpiraEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("AccessTokenProtegido")
                        .HasMaxLength(4000)
                        .HasColumnType("varchar(4000)");

                    b.Property<bool>("Ativa")
                        .HasColumnType("tinyint(1)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataConexao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("MetaUserId")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Nome")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("TokenType")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.HasKey("Id");

                    b.HasIndex("Ativa");

                    b.HasIndex("MetaUserId")
                        .IsUnique();

                    b.ToTable("MetaAdsContas", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsImagem", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AdAccountId")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<Guid>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("ContentHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("varchar(64)");

                    b.Property<string>("ContentType")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataUpload")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("MetaAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("MetaImageHash")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("NomeArquivo")
                        .IsRequired()
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("OrigemImagem")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<long?>("TamanhoBytes")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("MetaAdsContaId");

                    b.HasIndex("CampanhaId", "AdAccountId");

                    b.HasIndex("CampanhaId", "AdAccountId", "ContentHash")
                        .IsUnique();

                    b.ToTable("MetaAdsImagens", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsOAuthState", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataUtilizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("ExpiraEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("StateHash")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("varchar(128)");

                    b.Property<bool>("Utilizado")
                        .HasColumnType("tinyint(1)");

                    b.HasKey("Id");

                    b.HasIndex("ExpiraEm");

                    b.HasIndex("StateHash")
                        .IsUnique();

                    b.HasIndex("Utilizado");

                    b.ToTable("MetaAdsOAuthStates", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsPreparacaoPublicacao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AdAccountId")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<int>("AgeMax")
                        .HasColumnType("int");

                    b.Property<int>("AgeMin")
                        .HasColumnType("int");

                    b.Property<Guid>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("CountryCode")
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)");

                    b.Property<string>("CountryName")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("LocationKey")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("LocationName")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("LocationType")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<Guid>("MetaAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Region")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("RegionId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.HasKey("Id");

                    b.HasIndex("CampanhaId")
                        .IsUnique();

                    b.HasIndex("MetaAdsContaId");

                    b.ToTable("MetaAdsPreparacoesPublicacao", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsPublicacao", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AdAccountId")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("AdExternalId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("AdSetExternalId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("CampaignExternalId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<Guid>("CampanhaId")
                        .HasColumnType("char(36)");

                    b.Property<string>("CreativeExternalId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<DateTime?>("DataAtualizacao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataConclusao")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataInicio")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("FbTraceId")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<Guid>("MetaAdsContaId")
                        .HasColumnType("char(36)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<string>("UltimaEtapaConcluida")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("UltimoErroCodigo")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("UltimoErroHttpStatus")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)");

                    b.Property<string>("UltimoErroMensagem")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("UltimoErroSubcodigo")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("UltimoErroTipo")
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.HasKey("Id");

                    b.HasIndex("MetaAdsContaId");

                    b.HasIndex("Status");

                    b.HasIndex("CampanhaId", "AdAccountId")
                        .IsUnique();

                    b.ToTable("MetaAdsPublicacoes", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.OrigemLead", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AdGroupId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("AdId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("CampaignId")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Device")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Gbraid")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Gclid")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("IpHash")
                        .HasMaxLength(128)
                        .HasColumnType("varchar(128)");

                    b.Property<string>("Keyword")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("LandingPage")
                        .HasMaxLength(250)
                        .HasColumnType("varchar(250)");

                    b.Property<Guid>("LeadId")
                        .HasColumnType("char(36)");

                    b.Property<string>("MatchType")
                        .HasMaxLength(80)
                        .HasColumnType("varchar(80)");

                    b.Property<string>("Referrer")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("UserAgent")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<string>("UtmCampaign")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("UtmContent")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("UtmMedium")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("UtmSource")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("UtmTerm")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("Wbraid")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.HasKey("Id");

                    b.HasIndex("Gclid");

                    b.HasIndex("LandingPage");

                    b.HasIndex("LeadId")
                        .IsUnique();

                    b.HasIndex("UtmCampaign");

                    b.ToTable("OrigensLead", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Segment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("DefaultConfigJson")
                        .HasColumnType("json");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("Slug")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<string>("TemplateKey")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("varchar(120)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.HasIndex("IsActive");

                    b.HasIndex("Slug")
                        .IsUnique();

                    b.ToTable("Segments", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("3f1ce0a4-7ec5-4c8f-b6d9-df4f3e7f0c35"),
                            CreatedAt = new DateTime(2026, 9, 6, 0, 0, 0, 0, DateTimeKind.Utc),
                            DefaultConfigJson = "{\"businessCategory\":\"health_insurance_quote\",\"defaultGoal\":\"lead_generation_whatsapp\",\"legacyCompatibility\":true,\"contextDefaults\":{\"offerType\":\"quote\",\"primaryChannel\":\"whatsapp\",\"commercialApproach\":\"consultative\"},\"restrictions\":[\"nao_garantir_preco\",\"nao_garantir_cobertura\",\"nao_garantir_aprovacao\",\"nao_promover_carencia_zero\"],\"leadForm\":{\"submitButtonText\":\"Receber cotacao\",\"fields\":[{\"key\":\"name\",\"label\":\"Nome\",\"type\":\"text\",\"required\":true,\"order\":1},{\"key\":\"phone\",\"label\":\"WhatsApp\",\"type\":\"phone\",\"required\":true,\"placeholder\":\"(00) 00000-0000\",\"order\":2},{\"key\":\"quantidadeVidas\",\"label\":\"Quantidade de vidas\",\"type\":\"number\",\"required\":true,\"validation\":{\"min\":1,\"max\":999},\"order\":3}]}}",
                            Description = "Captação de leads para cotação consultiva de planos de saúde.",
                            IsActive = true,
                            Name = "Planos de Saúde",
                            Slug = "planos-saude",
                            TemplateKey = "high_ticket_quote"
                        });
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.TentativaCapturaLead", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CriadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("Duplicado")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Gclid")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("LandingPage")
                        .HasMaxLength(250)
                        .HasColumnType("varchar(250)");

                    b.Property<Guid?>("LeadId")
                        .HasColumnType("char(36)");

                    b.Property<string>("UtmCampaign")
                        .HasMaxLength(180)
                        .HasColumnType("varchar(180)");

                    b.Property<string>("WhatsAppNormalizado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id");

                    b.HasIndex("CriadoEm");

                    b.HasIndex("LeadId");

                    b.HasIndex("WhatsAppNormalizado");

                    b.ToTable("TentativasCapturaLead", (string)null);
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Campanha", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Segment", "Segment")
                        .WithMany("Campanhas")
                        .HasForeignKey("SegmentId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Segment");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CampanhaRevisao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany("Revisoes")
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campanha");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.ConfiguracaoSistemaHistorico", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.ConfiguracaoSistema", "ConfiguracaoSistema")
                        .WithMany("Historico")
                        .HasForeignKey("ConfiguracaoSistemaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ConfiguracaoSistema");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeAsset", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campaign")
                        .WithMany("CreativeAssets")
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campaign");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeAssetAnalysis", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.CreativeAsset", "CreativeAsset")
                        .WithMany("Analyses")
                        .HasForeignKey("CreativeAssetId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CreativeAsset");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeQualityOverride", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campaign")
                        .WithMany()
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.CreativeAssetAnalysis", "CreativeAssetAnalysis")
                        .WithMany()
                        .HasForeignKey("CreativeAssetAnalysisId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("LeadEngine.Domain.Entities.CreativeAsset", "CreativeAsset")
                        .WithMany()
                        .HasForeignKey("CreativeAssetId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Campaign");

                    b.Navigation("CreativeAsset");

                    b.Navigation("CreativeAssetAnalysis");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsAnaliseIa", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPublicacao", "Publicacao")
                        .WithMany()
                        .HasForeignKey("GoogleAdsPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Publicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsMetricaDiaria", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsConta", "GoogleAdsConta")
                        .WithMany()
                        .HasForeignKey("GoogleAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPublicacao", "Publicacao")
                        .WithMany()
                        .HasForeignKey("GoogleAdsPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("GoogleAdsConta");

                    b.Navigation("Publicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsOperacaoPublicacao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPublicacao", "Publicacao")
                        .WithMany("Operacoes")
                        .HasForeignKey("GoogleAdsPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Publicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPlanoPublicacao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany()
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsConta", "GoogleAdsConta")
                        .WithMany()
                        .HasForeignKey("GoogleAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Campanha");

                    b.Navigation("GoogleAdsConta");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPublicacao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany()
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsConta", "GoogleAdsConta")
                        .WithMany()
                        .HasForeignKey("GoogleAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPlanoPublicacao", "PlanoPublicacao")
                        .WithMany()
                        .HasForeignKey("GoogleAdsPlanoPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campanha");

                    b.Navigation("GoogleAdsConta");

                    b.Navigation("PlanoPublicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPublicacaoHistorico", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPublicacao", "Publicacao")
                        .WithMany("Historico")
                        .HasForeignKey("GoogleAdsPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Publicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsRecursoPublicado", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPublicacao", "Publicacao")
                        .WithMany("Recursos")
                        .HasForeignKey("GoogleAdsPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Publicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsSincronizacao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsConta", "GoogleAdsConta")
                        .WithMany()
                        .HasForeignKey("GoogleAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.GoogleAdsPublicacao", "Publicacao")
                        .WithMany()
                        .HasForeignKey("GoogleAdsPublicacaoId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("GoogleAdsConta");

                    b.Navigation("Publicacao");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Lead", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany("Leads")
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Campanha");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadAnswer", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Lead", "Lead")
                        .WithMany("Answers")
                        .HasForeignKey("LeadId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Lead");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadForm", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campaign")
                        .WithMany("LeadForms")
                        .HasForeignKey("CampaignId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Campaign");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadFormField", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.LeadForm", "LeadForm")
                        .WithMany("Fields")
                        .HasForeignKey("LeadFormId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LeadForm");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LogIntegracaoLead", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Lead", "Lead")
                        .WithMany("LogsIntegracao")
                        .HasForeignKey("LeadId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Lead");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsAtivoSelecionado", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.MetaAdsConta", "MetaAdsConta")
                        .WithOne("AtivoSelecionado")
                        .HasForeignKey("LeadEngine.Domain.Entities.MetaAdsAtivoSelecionado", "MetaAdsContaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MetaAdsConta");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsImagem", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany()
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.MetaAdsConta", "MetaAdsConta")
                        .WithMany()
                        .HasForeignKey("MetaAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Campanha");

                    b.Navigation("MetaAdsConta");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsPreparacaoPublicacao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany()
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.MetaAdsConta", "MetaAdsConta")
                        .WithMany()
                        .HasForeignKey("MetaAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Campanha");

                    b.Navigation("MetaAdsConta");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsPublicacao", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Campanha", "Campanha")
                        .WithMany()
                        .HasForeignKey("CampanhaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LeadEngine.Domain.Entities.MetaAdsConta", "MetaAdsConta")
                        .WithMany()
                        .HasForeignKey("MetaAdsContaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Campanha");

                    b.Navigation("MetaAdsConta");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.OrigemLead", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Lead", "Lead")
                        .WithOne("Origem")
                        .HasForeignKey("LeadEngine.Domain.Entities.OrigemLead", "LeadId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Lead");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.TentativaCapturaLead", b =>
                {
                    b.HasOne("LeadEngine.Domain.Entities.Lead", "Lead")
                        .WithMany()
                        .HasForeignKey("LeadId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Lead");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Campanha", b =>
                {
                    b.Navigation("CreativeAssets");

                    b.Navigation("LeadForms");

                    b.Navigation("Leads");

                    b.Navigation("Revisoes");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.ConfiguracaoSistema", b =>
                {
                    b.Navigation("Historico");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.CreativeAsset", b =>
                {
                    b.Navigation("Analyses");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.GoogleAdsPublicacao", b =>
                {
                    b.Navigation("Historico");

                    b.Navigation("Operacoes");

                    b.Navigation("Recursos");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Lead", b =>
                {
                    b.Navigation("Answers");

                    b.Navigation("LogsIntegracao");

                    b.Navigation("Origem")
                        .IsRequired();
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.LeadForm", b =>
                {
                    b.Navigation("Fields");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.MetaAdsConta", b =>
                {
                    b.Navigation("AtivoSelecionado");
                });

            modelBuilder.Entity("LeadEngine.Domain.Entities.Segment", b =>
                {
                    b.Navigation("Campanhas");
                });
#pragma warning restore 612, 618
        }
    }
}
